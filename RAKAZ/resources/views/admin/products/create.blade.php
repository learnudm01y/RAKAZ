...
@section('title', app()->getLocale() == 'ar' ? 'إضافة منتج جديد' : 'Add New Product')

@section('page-title')
    <span class="ar-text">إضافة منتج جديد</span>
    <span class="en-text">Add New Product</span>
@endsection
...
<div class="product-form-container">
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="product-form" id="product-form">
        @csrf
        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="sku" class="form-control @error('sku') is-invalid @enderror" value="{{ old('sku') }}" placeholder="PRD-{{ strtoupper(Str::random(8)) }}">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="stock_status" class="form-control" required>
                                    <option value="in_stock" {{ old('stock_status') == 'in_stock' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'متوفر' : 'In Stock' }}</option>
                                    <option value="out_of_stock" {{ old('stock_status') == 'out_of_stock' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'غير متوفر' : 'Out of Stock' }}</option>
                                    <option value="on_backorder" {{ old('stock_status') == 'on_backorder' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'طلب مسبق' : 'On Backorder' }}</option>
                                </select>
                            </div>

                            <div class="col-md-4 mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="manage_stock" id="manage_stock" {{ old('manage_stock', true) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-4 mb-3" id="low_stock_field">
                                ...
                                <input type="number" name="low_stock_threshold" class="form-control" value="{{ old('low_stock_threshold') }}" min="0">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4" id="colorImagesSection" style="display: none;">
                    <div class="card-header" style="cursor: pointer;" data-bs-toggle="collapse" data-bs-target="#colorImagesCollapse" aria-expanded="true">
                        <h3 class="card-title">
                            <i class="fas fa-images me-2"></i>
                            {{ app()->getLocale() == 'ar' ? 'صور الألوان' : 'Color Images' }}
                            <small class="text-muted ms-2" style="font-size: 0.75rem;">
                                {{ app()->getLocale() == 'ar' ? '(اختياري - أضف صورة لكل لون)' : '(Optional - Add image for each color)' }}
                            </small>
                        </h3>
                    </div>
                    <div class="collapse show" id="colorImagesCollapse">
                        <div class="card-body">
                            <div id="colorImagesContainer">
                                <!-- Color image cards will be dynamically added here -->
                            </div>
                            <div class="text-muted text-center py-3" id="noColorsSelectedMsg">
                                <i class="fas fa-info-circle me-2"></i>
                                {{ app()->getLocale() == 'ar' ? 'اختر ألواناً من القسم أعلاه لإضافة صور لها' : 'Select colors from the section above to add images' }}
                            </div>
                        </div>
                    </div>
                </div>
...
function updateColorImagesSection() {
    ...
    selectedColors.forEach(function(colorId) {
        if (container.find('[data-color-id="' + colorId + '"]').length === 0) {
            ...
            if (color) {
                var cardHtml = createColorImageCard(color);
...
function createColorImageCard(color) {
...
.color-image-input-wrapper {
    position: relative;
    margin-bottom: 0.75rem;
}

.color-image-input-wrapper input[type="file"] {
    position: absolute;
    opacity: 0;
    width: 100%;
    height: 100%;
...
.color-image-card.has-image .color-image-remove-btn {
    display: inline-flex;
    align-items: center;
    gap: 0.25rem;
}
...
.color-image-card.has-image .color-image-filename {
    display: block;
}
</style>
@endpush
