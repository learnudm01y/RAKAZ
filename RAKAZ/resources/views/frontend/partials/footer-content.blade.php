<footer class="main-footer">
    @php
        ...
        $footerSections = \App\Models\FooterSection::with(['activeItems'])->active()->ordered()->get();
        $socialLinks = \App\Models\FooterSocialLink::where('is_active', true)->orderBy('sort_order')->get();
        $footerSettings = \App\Models\FooterSetting::getAllSettings();

        // Check visibility settings
        $showNewsletter = $footerSettings['show_newsletter'] ?? true;
        $showSocialLinks = $footerSettings['show_social_links'] ?? true;
        $showAppsSection = $footerSettings['show_apps_section'] ?? true;
        $showContactInfo = $footerSettings['show_contact_info'] ?? true;

        // Contact info
        $customerServicePhone = $footerSettings['customer_service_phone'] ?? '800 717171';
        $whatsappNumber = $footerSettings['whatsapp_number'] ?? '+971 55 300 7879';

        // Newsletter settings
        $newsletterTitleAr = $footerSettings['newsletter_title_ar'] ?? 'اشترك في نشرتنا الإخبارية';
        $newsletterTitleEn = $footerSettings['newsletter_title_en'] ?? 'Subscribe to our newsletter';

        // Social title
        $socialTitleAr = $footerSettings['social_title_ar'] ?? 'تابع ركاز';
        $socialTitleEn = $footerSettings['social_title_en'] ?? 'Follow Rakaz';

        // Apps title
        $appsTitleAr = $footerSettings['apps_title_ar'] ?? 'تطبيقات ركاز';
        $appsTitleEn = $footerSettings['apps_title_en'] ?? 'Rakaz Apps';

        // Copyright
        $copyrightAr = $footerSettings['copyright_ar'] ?? 'ركاز LLC. 2025. جميع الحقوق محفوظة';
        $copyrightEn = $footerSettings['copyright_en'] ?? 'Rakaz LLC. 2025. All Rights Reserved';
    @endphp

    @if($showNewsletter || $showSocialLinks)
    <div class="footer-social-newsletter">
        @if($showNewsletter)
        <div class="newsletter-inline">
            <h3 class="newsletter-title">{{ $isAr ? $newsletterTitleAr : $newsletterTitleEn }}</h3>
        ...
        <div class="social-inline">
            <p class="social-title">{{ $isAr ? $socialTitleAr : $socialTitleEn }}</p>
            <div class="social-icons">
                @forelse($socialLinks as $social)
                <a href="{{ $social->url }}" class="social-icon" rel="noopener" target="_blank">
                    {!! $social->getIconSvg() !!}
                </a>
                @empty
    ...
    <div class="footer-content">
        ...
        @if($footerSections->count() > 0)
            ...
            <div class="footer-column">
                <h4 class="footer-title">{{ $section->getTitle($isAr ? 'ar' : 'en') }}</h4>
                <ul class="footer-links">
                    @foreach($section->activeItems as $item)
                    <li><a href="{{ $item->getUrl() }}">{{ $item->getDisplayTitle($isAr ? 'ar' : 'en') }}</a></li>
                    @endforeach
                </ul>
            </div>
            @endforeach
        @else
            {{-- Default static sections if no database data --}}
            <div class="footer-column">
        ...
        @if($showAppsSection)
        <div class="footer-column footer-apps-column">
            <p class="apps-title">{{ $isAr ? $appsTitleAr : $appsTitleEn }}</p>
            <div class="app-badges">
                    @php
                        ...
                        if ($homePage) {
                            ...
                            if (!empty($homePage->google_play_link) && $homePage->google_play_link != '#') {
                                $googlePlayLink = $homePage->google_play_link;
                            }

                            if (!empty($homePage->app_store_link) && $homePage->app_store_link != '#') {
                                $appStoreLink = $homePage->app_store_link;
                            }
                        }
                    @endphp
    ...
    <div class="footer-bottom">
        <div class="footer-info">
            ...
            <div class="footer-contact">
                <p>{{ $isAr ? 'خدمة العملاء:' : 'Customer Service:' }} <a href="tel:{{ preg_replace('/\s+/', '', $customerServicePhone) }}">{{ $customerServicePhone }}</a></p>
                <p>{{ $isAr ? 'خدمة واتساب:' : 'WhatsApp:' }} <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $whatsappNumber) }}">{{ $whatsappNumber }}</a></p>
            </div>
            @endif
            <div class="footer-copyright">
                <p>{{ $isAr ? $copyrightAr : $copyrightEn }}</p>
            </div>
        </div>
    </div>
</footer>
